...
export type ListDataFieldFilterValueType =
  | string
  | string[]
  | number
  | number[]
...
const normalizeFilters = (filters: ListDataFiltersType) =>
  omitBy(filters, isNull);
  ...
  const onFiltersChange = useCallback(
    (nextFilters: ListDataFiltersType | null) => {
      setParams((state: DataTableParamsInterface) => ({
        ...state,
        filters: nextFilters
          ? normalizeFilters({ ...state.filters, ...nextFilters })
          : {},
        pageNumber: 0,
      }));
    },
